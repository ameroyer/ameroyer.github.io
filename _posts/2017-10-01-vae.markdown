...
<span class='truc'>gloubi</span>
The `get_inputs_queue` function returns a queue whose elements are input dictionary with key "image": A 4D Tensor of size (BATCH_SIZE, HEIGHT, WIDTH, N_CHANNELS) representing the input images.
