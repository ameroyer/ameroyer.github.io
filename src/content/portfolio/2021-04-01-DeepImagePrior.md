...
twitter_summary: "A Keras/TensorFlow 2 implementation of Deep Image Prior and Deep Decoder for image restoration."
---
...
$$
\text{Randomly initialize a neural network $f_{\theta}$ and an input $z$}\\
\text{Train parameters }\theta^\ast = \arg\min_{\theta} E(f_{\theta}(z),x_0) \\
\text{Output }x^\ast = f_{\theta^\ast}(z)
$$
